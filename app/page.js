        ...
        <div className="create-card animate-in" style={{animationDelay:'0.12s'}}>
          ...
          <div className="create-card-form">
            ...
            <select value={type} onChange={e => setType(e.target.value)}>
              <option value="round-robin">League (Round Robin)</option>
              <option value="knockout">Knockout Cup</option>
            </select>
            <select value={legs} onChange={e => setLegs(+e.target.value)} style={{minWidth:80}}>
              <option value={1}>1 leg</option>
              <option value={2}>2 legs</option>
              <option value={0}>Custom</option>
            </select>
            {legs === 0 && (
              <input type="number" min="1" max="99" placeholder="Games" value={legsCustom} onChange={e => setLegsCustom(e.target.value)} style={{width:70}} />
            )}
          ...
          <div className="league-list stagger-children">
            {leagues.map(l => (
              <Link key={l.id} href={'/leagues/' + l.id} className="league-card" style={{textDecoration:'none'}}>
                ...
                <div className="lc-icon" style={{fontSize:28}}>{l.type === 'knockout' ? '\u{1F3C6}' : '\u26BD'}</div>
